// Apply Supabase schema via Node + pg
...
async function main() {
  ...
  if (!uri) {
    console.error('Usage: node server/sql/apply_schema.mjs <postgresql://...>');
    process.exit(1);
  }
  const sql = await fs.readFile('server/sql/supabase_schema.sql', 'utf8');
