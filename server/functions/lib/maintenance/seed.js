...
function assertAuthorized(req) {
    var _a;
    const secret = process.env.SEED_SECRET || ((_a = functions.config().seed) === null || _a === void 0 ? void 0 : _a.secret);
    const token = req.query.secret || req.header('x-seed-secret');
    if (!secret || token !== secret) {
        throw new functions.https.HttpsError('permission-denied', 'Unauthorized');
    }
}
exports.seedInitialData = functions.region('us-central1').https.onRequest(async (req, res) => {
    try {
        assertAuthorized(req);
        const db = admin.firestore();
        // Helper to create doc only if collection is empty
        const ensureDocs = async (colPath, docs) => {
            ...
            docs.forEach((d) => {
                ...
                batch.set(ref, Object.assign(Object.assign({}, d), { id: d.id || ref.id }));
            });
            await batch.commit();
        ...
        const audioUrl = (name) => `https://storage.googleapis.com/${bucket}/audio/${name}`;
        const imageUrl = (name) => `https://storage.googleapis.com/${bucket}/images/${name}`;
        ...
        const status = (err === null || err === void 0 ? void 0 : err.code) === 'permission-denied' ? 403 : 500;
        res.status(status).json({ ok: false, error: (err === null || err === void 0 ? void 0 : err.message) || 'Unknown error' });
    }
});
